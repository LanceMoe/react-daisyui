...
import type { ComponentBrandColors, ComponentStatus, IComponentBaseProps } from '../types';
...
const DockItem = forwardRef<HTMLButtonElement, DockItemProps>(
  ({ children, className, color, dataTheme, active, disabled, ...props }, ref): React.JSX.Element => {
    ...
    return (
      <button {...props} className={classes} data-theme={dataTheme} disabled={disabled} ref={ref}>
        {children}
      </button>
    );
  },
