...
describe('RadialProgress', () => {
  ...
  it('Should apply additional class names', () => {
    const { container } = render(<RadialProgress className="custom-class" value={50} />);
    expect(container.firstChild).toHaveClass('custom-class');
  });

  it('Should pass down data-theme prop', () => {
    const { container } = render(<RadialProgress dataTheme="light" value={50} />);
  ...
  it('Should apply size and thickness styles correctly', () => {
    const { container } = render(<RadialProgress size="6rem" thickness="6px" value={50} />);
