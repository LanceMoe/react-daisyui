...
describe('ChatBubble', () => {
  ...
  test('Should render ChatBubbleHeader subcomponent', () => {
    const { getByText } = render(<ChatBubble.Header>Header Content</ChatBubble.Header>);
    expect(getByText('Header Content')).toBeInTheDocument();
  });

  test('Should render ChatBubbleTime subcomponent', () => {
    const { getByText } = render(<ChatBubble.Time>Time Content</ChatBubble.Time>);
  ...
  test('Should render ChatBubbleMessage subcomponent', () => {
    const { getByText } = render(<ChatBubble.Message>Message Content</ChatBubble.Message>);
    expect(getByText('Message Content')).toBeInTheDocument();
  });

  test('Should render ChatBubbleFooter subcomponent', () => {
    const { getByText } = render(<ChatBubble.Footer>Footer Content</ChatBubble.Footer>);
